...
var trendJs = (function () {
  ...
  trendJs.initTrendViewsOption = function () {
    var option = {
      ...
      yAxis: { name: "" },
      series: [
        { name: "value", type: "line", symbol: "emptycircle", data: [] },
      ],
    };
    return option;
  };
  ...
  function getVrendViewItemDataName(obj) {
    ...
    if (obj.attr("block_type_name").indexOf("V") !== -1) {
      var blockNameWithNumber =
        obj.attr("block_type_name") + obj.attr("block_Number");
      itemName =
        obj.attr("vm_name") +
        " (" +
        blockNameWithNumber +
        obj.attr("block_format").substring(0, 1).toLowerCase() +
        getDecimalIndexFromValue(obj.attr("decimal")) +
        ")";
    } else {
      itemName =
        obj.attr("vm_name") +
        " (" +
        obj.attr("block_Number") +
        obj.attr("block_format").substring(0, 1).toLowerCase() +
        getDecimalIndexFromValue(obj.attr("decimal")) +
        ")";
    }
 ...
 * */
if (renderJs) {
  renderJs.registerCustomizeComponentRuntimeInitMethod(
    "trendJs.initTrendViews()"
  );
}
