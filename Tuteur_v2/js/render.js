...
var renderJs = (function () {
  ...
  function initVmByHttp() {
    var path = renderJs.isBmBaseOnV84()
      ? "tagtable.vm"
      : "/lfs/dev/sdcard/webroot/tagtable.vm";
    if (IS_CLOUD_MODE) {
      path = "tagtable.vm";
    }
    $.ajax({
  ...
  renderJs.initBindData = function (vmArray) {
    $(".server_binding").each(function (index, div) {
      for (var j = 0; j < vmArray.length; j++) {
        if (
          div.getAttribute("tag_id") &&
          vmArray[j].id === div.getAttribute("tag_id")
        ) {
  ...
  renderJs.initNavivator = function () {
    ...
    if (!isHttpsProtocol()) {
      ...
      var paraStr =
        "?!App-Language=" +
        LocalStorage.Instance().Get(
          LOCAL_STORAGE_PREFIX + "_current_language"
        ) +
        "&Security-Hint=" +
        LocalStorage.Instance().Get(
          LOCAL_STORAGE_PREFIX + "_current_login_ref"
        ) +
        "&rdm=" +
        rdm;
  ...
  function navButtonReposition() {
    ...
    if (windowHeight < mainHeight) {
      var newTop =
        (windowHeight - tagHeight) / 2 + $(document).scrollTop() - 55;
      $("#tag").css("top", newTop);
    } else {
  ...
  renderJs.navGo = function (obj) {
    ...
    var loginString =
      "?!App-Language=" +
      LocalStorage.Instance().Get(LOCAL_STORAGE_PREFIX + "_current_language");
    if (!isHttpsProtocol()) {
      //http
      loginString =
        loginString +
  ...
  renderJs.openLinkInNewWindow = function (obj) {
    ...
    window.open(
      target,
      "newwindow",
      "height=800,width=800,top=200,left=500,toolbar=no,menubar=no,scrollbars=yes, resizable=yes,location=yes, status=no"
    );
  };
  ...
  renderJs.initialAnalogSliderComp = function () {
    ...
    sliders.each(function (index) {
      ...
      if (hdl_img) {
        $(parent)
          .children(".analog_slider_comp")
          .children(".ui-slider-handle")
          .css("background-image", "url(" + getImageByProtocol(hdl_img) + ")");
      }
      var bar_img = $(parent).attr("bar_img");
      if (bar_img) {
        $(parent)
          .children(".analog_slider_comp")
          .css("background-image", "url(" + getImageByProtocol(bar_img) + ")");
      }
      ...
      if ($(parent).attr("mode") === "true") {
        $(this)
          ...
          .rotateSlide({
            change: function (event, ui) {
              ...
              if ($(parent).attr("event_mode") === "write") {
                var newValue = renderJs.getIntValueFromAanlogInput(
                  $(parent),
                  ui.value
                );
          ...
          .children(".ui-slider-handle")
          .css("margin-top", "0"); //.attr({title: midVal})
      } else {
        $(this).rotateSlide({
          change: function (event, ui) {
            $(this).children(".ui-slider-handle").removeAttr("title");
  ...
  function showAnalogValueSubmitBtn(bindObj, inputBox, placeHolder) {
    ...
    btn.attr(
      "style",
      "width:26px;position:absolute;top:0;color:green;font-weight:bold;"
    );
    btn.css("left", x);
    ...
    if (inputVal !== "") {
      if ($(bindObj).attr("comp_type") === "AnalogTimeComponent") {
        if (renderJs.isAnalogTimeValid($(bindObj), inputVal)) {
          ...
          if (
            $(bindObj).attr("time_format") === "md" ||
            $(bindObj).attr("time_format") === "dm"
          ) {
            // [yearly] time_format==HEX
            $(bindObj).attr("block_format", "Hex");
          } else if ($(bindObj).attr("time_format") === "hm") {
            // [weekly]
  ...
  renderJs.getAtcValFromAnalogTime = function (obj, input) {
    ...
    if (timeUnit === "m") {
      ...
      atcVal =
        parseInt(time[1]) * 6000 + parseInt(time[2]) * 100 + parseInt(hms[1]);
  ...
  renderJs.setAnalogData = function (obj) {
    ...
    var slider_maxVal = parseInt(
      $("#" + id)
        .children(".analog_max_val")
        .contents()[0].data
    );
    var slider_minVal = parseInt(
      $("#" + id)
        .children(".analog_min_val")
        .contents()[0].data
    );
    var currentVal = parseInt(
      $("#" + id)
        .children(".analogValue")
        .val()
    );
    $("#" + id)
      .children(".analog_bar")
    ...
    $("#" + id)
      ...
      .hide();
    $("#" + id)
      .children(".setAnalogBtn")
      .show();
    var slider = $("#" + id)
      .children(".analog_set_val")
      .css("margin", "10 auto")
      .show()
      .rotateSlide({
        orientation: "vertical",
  ...
  renderJs.updateNoticeDisplay = function (divID, value) {
    if ($("#" + divID).has(".analog_input")) {
      var xAxis =
        $("#" + divID).attr("text_align") === "right" ? "left" : "right";
      $("#" + divID)
        .children(".notice")
        .css("display", "inline")
        .css("position", "absolute")
  ...
  renderJs.showAnalogBar = function (id, maxVal, minVal, strValue) {
    ...
    if (strValue >= maxVal) {
      ...
      whiteHeight =
        Math.round((1 - (strValue - minVal) / (maxVal - minVal)) * 10000) /
          100.0 -
        0.1 +
        "%";
    }
  ...
  renderJs.getValueByBlockFormat = function (
    blockFormat,
    accessModeType,
    sValue
  ) {
  ...
  function checkBM_connection(callBackParameter) {
    AjaxRequest.Instance().Request(
      ...
      "SYM",
      DESMakeKey(
        LocalStorage.Instance().Get("logo_current_login_key1A1"),
        LocalStorage.Instance().Get("logo_current_login_key1A2")
      ),
      DESMakeKey(
        LocalStorage.Instance().Get("logo_current_login_key1B1"),
        LocalStorage.Instance().Get("logo_current_login_key1B2")
      ),
      LocalStorage.Instance().Get("logo_current_login_ref")
    );
  }
  function connectionCallBack(param) {
    if (this.m_iLocalResult == 601 || this.m_iLocalResult == 0) {
      //timeout:
      $("#timeout_dlg").show();
      $("#timeout_dlg")
        .dialog({
  ...
  renderJs.initUserInfo = function () {
    ...
    $("#nav .menuTitle").append(
      "<div id='userInfo' style='font-size:14;font-weight:100'>[ " +
        currentUser +
        " ]</div>"
    );
  };
  ...
  renderJs.onPushButtonDown = function (oTarget) {
    ...
    if (!isWaiting || isWaiting === "-1") {
      ...
      var pushbuttonIntervalId = setInterval(
        renderJs.setPushButtonValue,
        40,
        oTarget
      );
  ...
  renderJs.onPushButtonUp = function (oTarget) {
    ...
    if (
      mouse_mode !== "Hold" ||
      pushbuttonIntervalId === "" ||
      pushbuttonIntervalId === undefined
    ) {
      return true;
    }
  ...
  function parseTimeFromBmStr(sv) {
    let sec =
      renderJs.getValueByBlockFormat("Unsigned", "2", sv.substr(0, 2)) + "";
    ...
    let dateTimeStr =
      year + "/" + month + "/" + day + " " + huor + ":" + minute + ":" + sec;
  ...
  function parseVerStr(sFWVersion) {
    ...
    if (
      sFWVersion &&
      sFWVersion.split(" ").length === 3 &&
      sFWVersion.indexOf("v1.8") !== -1
    ) {
      //logo! bm v1.84.01.2037 master
      version = parseInt(
        sFWVersion.split(" ")[2].replace(/\./g, "").substring(1, 6)
      );
    }
    return version;
  }
  renderJs.getFwVersion = function () {
    ...
    if (isHttpsProtocol()) {
      ...
      return window.location.pathname.indexOf("/lfs/dev/sdcard/") !== -1
        ? 18301
        : 18401;
    } else {
  ...
  renderJs.isBm83HttpsMode = function () {
    ...
    return (
      !IS_CLOUD_MODE &&
      window.location.pathname.indexOf("/lfs/dev/sdcard/webroot/https/") !== -1
    );
  };
...
function _callback_loadAnalogBarComponent() {
  ...
  var value = renderJs.getValueByBlockFormat(
    blockFormat,
    accessModeType,
    this.m_sValue
  );
...
function _callback_loadAnalogSliderComponent() {
  ...
  if (STOP_SLIDER_REFERSH_ID !== id) {
    ...
    $("#" + id)
      .children(".analog_slider_comp")
      .rotateSlide("value", value);
    var spHeight = $("#" + id + " div")
      .children(".ui-slider-handle")
      .css("height");
    var sliderComWd = parseFloat(
      $("#" + id)
        .children(".analog_slider_comp")
        .css("width")
    );
    var sliderhanWd = parseFloat(
      $("#" + id + " div")
        .children(".ui-slider-handle")
        .css("width")
    );
...
function _callback_loadAnalogTimeComponent() {
  ...
  var signedValue = Unsigned2Signed(
    parseInt(this.m_sValue, 16),
    accessModeType
  );
  if (signedValue < 0) {
  ...
  if (time_format === "md") {
    ...
    actVal = renderJs.getValueByBlockFormat(
      blockFormat,
      accessModeType,
      this.m_sValue
    );
...
function _callback_loadCustomizedComponent() {
  ...
  document.getElementById(id).style.backgroundImage =
    "url(" + getImageByProtocol(showImage) + ")";
}
