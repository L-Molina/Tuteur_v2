...
_callback_rainbowOnChange = function () {
  ...
  var actVal = renderJs.getValueByBlockFormat(
    blockFormat,
    accessModeType,
    this.m_sValue
  );
...
var rainbowJs = (function () {
  ...
  rainbowJs.initChart = function (compID) {
    ...
    if ($("#wrap").length === 0) {
      //editTime
      myChart = echarts.init(document.getElementById(compID), null, {
        renderer: "svg",
      });
    } else {
      //runTime
      ...
      series: [
        {
          ...
          axisTick: { length: 10, lineStyle: { color: "auto", shadowBlur: 0 } },
          splitLine: {
            length: 15,
            lineStyle: { width: 2, color: "auto", shadowBlur: 0 },
          },
          data: [{ value: 0, name: "Analogdata" }],
        },
      ],
    };
  ...
  rainbowJs.updateAnalogRainbowLineStyle = function (id, jsonData) {
    ...
    for (var i = 0; i < json.scopeColorList.length; i++) {
      ...
      colors[i][1] = this.calculaterJavaColorToJsColor(
        json.scopeColorList[i].color.value
      );
    }
    option.series[0].axisLine.lineStyle.color = colors;
    myChart.setOption(option, true);
