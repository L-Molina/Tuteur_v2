...
function Edit() {
    ...
    sliders.each(function (index) {
      ...
      if (hdl_img) {
        $(parent)
          ...
          .css("background-image", "url(" + hdl_img + ")");
      }
      var bar_img = $(parent).attr("bar_img");
      if (bar_img) {
        $(parent)
          .children(".analog_slider_comp")
          .css("background-image", "url(" + bar_img + ")");
      }
    });
  };
  ...
  this.resizeDiv = function (id, width, height) {
    ...
    target.css(
      "background-size",
      this.parseToInt(width) + "px " + (this.parseToInt(height) + 1) + "px"
    );
    ...
    if (target.hasClass("textCenter")) {
      if (
        target.attr("comp_type") === "CustomizedComponent" ||
        target.attr("comp_type") === "PushButton"
      ) {
        if ($("#" + id + " p").length === 0) {
          //target.children(":first")[0].tagName=='p'
  ...
  this.resizePageDiv = function (id, width, height) {
    ...
    if (
      ...
      target.css("background-image").indexOf("linear-gradient") != -1
    ) {
      //background grid
      target.css(
        "background",
        "linear-gradient(60deg, white 0px, black 1px, red 0px, white 0px) 25px -24px"
      );
  ...
  this.addDiv = function (id, x, y, htmlStr, script) {
    ...
    var newNode =
      this.getBody().childNodes[this.getBody().childNodes.length - 1];
    newNode.id = id;
    var target = $("#" + id);
    ...
    if (
      target.attr("comp_type") &&
      target.attr("comp_type").indexOf("AnalogSliderComponent") != -1
    ) {
      ...
      sliderBar.css(
        "background-size",
        sliderBar.width() + "px " + sliderBar.height() + "px"
      );
    }
  ...
  this.setEditReference = function () {
    $(document.createElement("script"))
      ...
      .attr("src", "res/js/jquery-1.11.2.min.js")
      .appendTo("head")
      .after("\n");
    $(document.createElement("script"))
      .attr("type", "text/javascript")
      .attr("src", "res/js/jquery-ui/jquery-ui.min.js")
      .appendTo("head")
      .after("\n");
    $(document.createElement("script"))
      .attr("type", "text/javascript")
      .attr("src", "res/js/edit.js")
      .appendTo("head")
      .after("\n");
    $(document.createElement("script"))
      .attr("type", "text/javascript")
      .attr("src", "res/js/bootstrap2-toggle.js")
      .appendTo("head")
      .after("\n");
    $(document.createElement("script"))
      .attr("type", "text/javascript")
      .attr("src", "res/js/bootstrap.min.js")
      .appendTo("head")
      .after("\n");
    $("body").attr("onload", "");
  };
  this.setRuntimeReference = function () {
    $(document.createElement("script"))
      .attr("type", "text/javascript")
      .attr("src", "/js/storage.js")
      .appendTo("head")
      .after("\n");
    $(document.createElement("script"))
      .attr("type", "text/javascript")
      .attr("src", "/js/utility.js")
      .appendTo("head")
      .after("\n");
    $(document.createElement("script"))
      .attr("type", "text/javascript")
      .attr("src", "/js/bigint.js")
      .appendTo("head")
      .after("\n");
    $(document.createElement("script"))
      .attr("type", "text/javascript")
      .attr("src", "/js/encrypt.js")
      .appendTo("head")
      .after("\n");
  ...
  this.updateLinkTargetInfo = function (
    id,
    linkType,
    url,
    uniqueName,
  ...
  this.showTestMessage = function (message) {
    $("#testMessage").remove();
    $("#main").append(
      "<div id='testMessage' style='position: absolute; left:0px; top: 0px; '>" +
        message +
        "</div>"
    );
  };
