...
Edit.prototype = {
  ...
  updateAnalogTextDemo: function (id, analogTextDemoStr, symbol, place) {
    var analogTextVal = $("#" + id)
      .children(".valueText")
      .children(".value");
    var analogInputPlaceholder = $("#" + id)
      .children(".valueText")
      .children(".inputVal");
    if (analogTextVal) analogTextVal.html(analogTextDemoStr);
  ...
  updateTimeFormat: function (id, timeUnit) {
    ...
    if (timeUnit === "h") {
      ...
      $("#" + id)
        .children(".valueText")
        .children(".inputVal")
        .attr("placeholder", "00:00:00.0");
      $("#" + id).attr("title", "hh:mm:ss.ms");
    } else if (timeUnit === "md") {
      //for yearly month-day
      $("#" + id)
        .children(".valueText")
        .children(".inputVal")
        .attr("placeholder", "month - day");
  ...
  resizeAnalogDiv: function (id, width, height) {
    ...
    if (sliderBar) {
      ...
      sliderHdl
        .width(rSize)
        .height(rSize)
        .css("background-size", rSize + "px " + rSize + "px");
      ...
      sliderBar.css(
        "background-size",
        sliderBarWidth + "px " + sliderBarHeight + "px"
      );
    }
    if (valueText) {
      $("#" + id)
      ...
      unit.css(
        "top",
        (editJs.parseToInt(height) - editJs.parseToInt(fontSize)) / 2
      );
    }
  },
  ...
  updateSliderHandlerImage: function (id, imgPath) {
    ...
    sliderHdl
      ...
      .css(
        "background-size",
        sliderHdl.width() + "px " + sliderHdl.height() + "px"
      );
  },
  //-----------------AnalogBar--------------------------------------
  ...
  setAnalogBarScale: function (id, maxVal, minVal, minScale) {
    ...
    maxVal = !maxVal
      ? parseFloat($("#" + id).attr("max_val"))
      : parseFloat(maxVal);
    minVal = !minVal
      ? parseFloat($("#" + id).attr("min_val"))
      : parseFloat(minVal);

    var scaleValueFixedNum = this.scaleValueDecimalPlace(
      maxVal,
      minVal,
      decimal
    );
    ...
    $("#" + id + " ul").append(
      "<li style='height:1px; top:" + top + "px'><a>" + minVal + "</a></li>"
    );
    $("#" + id + " ul li a").css({
  ...
  updateAnalogScaleTimeFontStyle: function (
    id,
    fontFamily,
    fontStyle,
    fontSize
    ...
    var top =
      (parseInt($("#" + id).css("height")) - parseInt(unitfontSize)) / 2;
    unit.css("top", top);
  },
  ...
  updateAnalogDateTimeFormat: function (id, format) {
    ...
    comp
      .children("div:first")
      .append(
        "<div style='position:relative;display: table-cell; vertical-align:middle;' class='outer'></div>"
      );
